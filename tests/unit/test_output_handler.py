...
from voice_typing.config.schema import AppConfig
from voice_typing.output.handler import OutputHandler
...
def test_inject_text_uses_first_available_strategy() -> None:
    ...
    with patch("voice_typing.output.strategies.keyboard.keyboard") as kb:
        kb.write = MagicMock()
        kb.send = MagicMock()
...
def test_fallback_to_clipboard_when_keyboard_fails() -> None:
    ...
    with patch("voice_typing.output.strategies.keyboard.keyboard") as kb, \
         patch("voice_typing.output.strategies.clipboard.pyperclip") as pc, \
         patch("voice_typing.output.strategies.clipboard.keyboard") as clip_kb:
